...
public class SmoodChangeValueManipulator implements Manipulator<ChangeValueManipulation> {
	 ...
	 * <p>
	 * Smood updates it's indices based on {@link ChangeValueManipulation}s as it listens to the changes done by this manipulator. Now, if an error
	 * happens while processing this notification, we might end up in an invalid state, because the index was not updated, but the property of the
	 * entity was already changed (because the {@link ManipulationTrackingPropertyAccessInterceptor}) first applies the change, then notifies
	 * listeners). This method therefore fixes it by setting back the original value.<br>
	 * <p>
	 * For this reason, the Smood's listener registers itself as the first listener in the chain, and for this to work properly, no listener on
	 * Smood's session ever should register itself as first. Otherwise it's gonna be notified with changes that might be undone silently.<br>
	 * <p>
	 * This method is especially important if id property of a new entity is assigned to an already used id. If we didn't set the property back, the
	 * subsequent undoing of the already-applied manipulations would not work. An attempt to undo the instantiation, thus removing the entity from the
	 * population manager, would fail, as the entity is indexed with id being <tt>null</tt>, but the entity that is being removed from population
	 * manager has id assigned.
	 */
	private void assignPropertySafely(ManipulatorContext context, GenericEntity entity, Object newValue, Property property) {
		...
		Object resolvedNewValue = context.resolveValue(property.getType(), newValue);

		try {
			property.set(entity, resolvedNewValue);
