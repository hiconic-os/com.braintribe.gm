...
public class GraphPrototypePai extends PropertyAccessInterceptor {
	...
	private Object createDefault(GenericModelType type) {
		switch (type.getTypeCode()) {
			case entityType:
				return createDefaultEntity((EntityType<?>) type);
			case setType:
				return createDefaultCollection((SetType) type);
			case listType:
				return createDefaultCollection((ListType) type);
			case mapType:
				return createDefaultMap((MapType) type);
			default:
	...
	private Collection<?> createDefaultCollection(LinearCollectionType type) {
		...
		Object defaultObject = createDefault(elementType);
		if (defaultObject != null) {
			collection.add(defaultObject);
		}
		return collection;
	}
