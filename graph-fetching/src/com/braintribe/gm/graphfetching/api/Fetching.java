 ...
 * <pre>
 *   EntityGraphNode fetchPlan = Fetching.rootNode(MyType.T,
 *       Fetching.node("referenceEntity",
 *           Fetching.node("someDeepProperty")
 *       )
 *   );
 *   Fetching.fetch(session, fetchPlan, entities);
 * </pre>
...
public interface Fetching {
    /**
     * Creates a prototype instance for the given entity type, suitable for specifying a fetch graph via property access.
     * All properties accessed on this instance will become part of the implicit fetch plan.
     ...
     * @param subNodes optional subnodes (for nested fetches)
     ...
     * @param entityType the root entity's type
     * @param subNodes optional subnodes (property branches)
     ...
     * @param selectPrototype prototype instance created by {@link #graphPrototype(EntityType)}
     * @return a new EntityGraphNode representing the root of the fetch graph
     */
     ...
     * @param session the PersistenceGmSession (detached mode)
     * @param node the fetch graph definition (root)
    ...
    /**
     * Populates the entity graph for the given entities within the specified session. Entities must belong to the session or be detached.
     * Handles both attached and detached scenarios
     * @param session PersistenceGmSession
     * @param node The graph of properties/entities to fetch (created via rootNode/Prototypes)
     * @param entities The root entities to populate
     */
