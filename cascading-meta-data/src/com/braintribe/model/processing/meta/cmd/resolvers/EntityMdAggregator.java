...
public class EntityMdAggregator extends MdAggregator {
	...
	private final ReentrantLock relevantSuperEntityMdIndicesLock = new ReentrantLock();
	...
	private Map<Class<? extends SelectorContextAspect<?>>, Object> localApects() {
		...
		Map<Class<? extends SelectorContextAspect<?>>, Object> result = newMap();
	...
	private void ensureRelevantEntityMdIndicesSync() {
		relevantSuperEntityMdIndicesLock.lock();
		try {
			if (relevantSuperEntityMdIndices != null) {
				return;
			}

			List<EntityMdIndex> indices = newList();
			...
			relevantSuperEntityMdIndicesLock.unlock();
		}
	}
