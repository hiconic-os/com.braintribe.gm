	...
	public void run() throws GmTraversingException {
		while (currentEvent != null) {
			...
			if (currentEvent.isSkippedByParent()) {
				...
				TraversingModelPathElement currentPathElement = currentEvent.getPathElement();
				...
				if (currentEvent.isEnter()) {
					if (skipAll) {
						...
						for (GmTraversingVisitor visitor : visitors)
							visitor.onElementEnter(this, currentPathElement);

						currentDepth++;
					}
				} else { // if current Event is a Leave event
					for (GmTraversingVisitor visitor : visitors)
						visitor.onElementLeave(this, currentPathElement);
			...
			currentEvent = nextEvent != null ? nextEvent : //
					(!currentEvent.isSkippedByParent() && currentEvent.isEnter() && skipAll ? nextEvent : currentEvent.next);
		}
	}
