...
public class BasicAccessAwareRequestContext<P extends AccessRequest> implements AccessRequestContext<P> {
	...
	public BasicAccessAwareRequestContext(ServiceRequestContext delegate, PersistenceGmSessionFactory requestSessionFactory, PersistenceGmSessionFactory systemSessionFactory, P originalRequest) {
	...
	public P getRequest() {
		if (request == null) {
			request = getSession().merge().adoptUnexposed(false).envelopeFactory(new TransientDataAwareEnvelopeFactory()).suspendHistory(true).doFor(originalRequest);
		}
		return request;
	}
	...
	public P getSystemRequest() {
		if (systemRequest == null) {
			systemRequest = getSystemSession().merge().adoptUnexposed(false).envelopeFactory(new TransientDataAwareEnvelopeFactory()).suspendHistory(true).doFor(originalRequest);
		}
