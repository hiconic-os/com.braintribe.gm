...
import com.braintribe.xml.stax.parser.test.model.Container;
import com.braintribe.xml.stax.parser.test.model.Grouping;

public class StaxModelParserTest {
	...
	private <T> T test(File file, ContentExpertRegistry registry) {
		try {
			...
			StaxModelParser parser = new StaxModelParser();
	...
	public void testSimple() {
		StaxParserTestRegistry registry = new StaxParserTestRegistry();
	...
	public void testAutoProcessing() {
		...
		// complex
		List<com.braintribe.xml.stax.parser.test.model.Entry> entries = container.getEntries();
		Assert.assertTrue("expected [" + 2 + "] complex entries, found [" + entries.size() + "]" , entries.size() == 2);
		
		com.braintribe.xml.stax.parser.test.model.Entry entry1 = entries.get(0);
		...
		com.braintribe.xml.stax.parser.test.model.Entry entry2 = entries.get(1);
