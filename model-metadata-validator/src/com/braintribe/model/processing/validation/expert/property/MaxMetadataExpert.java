...
public class MaxMetadataExpert implements PropertyValidationExpert {

	@Override
	public void validate(PropertyValidationContext context) {
		Object propertyValue = context.getPropertyValue();
		if (propertyValue instanceof Number) {
			Max max = context.getPropertyMdResolver().meta(Max.T).exclusive();
			if (max != null) {

				Object limit = max.getLimit();
				boolean constraintViolated;

				if (limit.getClass() != propertyValue.getClass()) {
					context.notifyConstraintViolation("Error while resolving " + Max.T.getShortName() + " metadata: Property has a different class ("
				...
				switch (context.getPropertyType().getTypeCode()) {
					...
					case doubleType:
						...
						break;
					default:
						throw new IllegalStateException("Found " + max.entityType().getTypeSignature() + " metadata on a property of type "
								+ context.getEntityType().getTypeSignature() + " which is not supported.");
				}

				if (constraintViolated) {
					context.notifyConstraintViolation("Property exceeds its allowed maximum value. Max: " + limit);
				}
			}
		}
	}
