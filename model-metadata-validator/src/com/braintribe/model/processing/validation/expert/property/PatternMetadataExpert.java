...
public class PatternMetadataExpert implements PropertyValidationExpert {
	...
	public void validate(PropertyValidationContext context) {
		...
		if (propertyValue instanceof String) {
			...
			Pattern pattern = context.getPropertyMdResolver().meta(Pattern.T).exclusive();
			if (pattern != null) {
				String expression = pattern.getExpression();

				if (!propertyStringValue.matches(expression)) {
					context.notifyConstraintViolation("String does not match the following RegEx: '" + expression + "'");
				}
