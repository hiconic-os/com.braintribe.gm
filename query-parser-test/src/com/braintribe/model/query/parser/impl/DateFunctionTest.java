...
public class DateFunctionTest extends AbstractQueryParserTest {
	...
	public void testNow() throws Exception {
		...
		SelectQuery expectedQuery = sq()
				.from(Person.class, "p")
				.where()
					.asString().lower().property("p", "name").eq().value(Now.T.create())
				.done();
		// @formatter:on
		// this is needed as the builder does not provide the required functionality
		((ValueComparison) expectedQuery.getRestriction().getCondition()).setRightOperand(Now.T.create());

		String queryString = "select * from " + Person.class.getName() + " p where toString(lower(p.name)) = now()";
