...
public class DDSA implements ServiceProcessor<ServiceRequest, Object>, ProceedContext {
	...
	private static abstract class AbstractDdsaInterceptionBuilder implements ServiceInterceptionChainBuilder {
		final protected DDSA chain;

		private AbstractDdsaInterceptionBuilder() {
			chain = new DDSA();
		}
	...
	private static class ProceedContextBuilderImpl extends AbstractDdsaInterceptionBuilder implements ProceedContextBuilder {
	...
	private static class InterceptingServiceProcessorBuilderImpl extends AbstractDdsaInterceptionBuilder
	...
	protected ProceedContext newProceedContext(ServiceRequestContext requestContext) {
		return new _ProceedCtx(requestContext, 0);
	}

	private class _ProceedCtx implements ProceedContext, ServiceProcessor<ServiceRequest, Object> {
		...
		public Object process(ServiceRequestContext requestContext, ServiceRequest request) {
			...
			_ProceedCtx nextProceedCtx = new _ProceedCtx(requestContext, index + 1);
		...
		public <T> T proceed(ServiceRequestContext context, ServiceRequest serviceRequest) {
			...
			try {
				...
				// call the next around processor with the next proceed context
				ServiceAroundProcessor<ServiceRequest, ?> nextAroundProcessor = aroundProcessors.get(index);
				_ProceedCtx nextProceedCtx = new _ProceedCtx(context, index + 1);
