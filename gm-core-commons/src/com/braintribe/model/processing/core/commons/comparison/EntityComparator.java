...
public class EntityComparator implements Comparator<GenericEntity> {
	...
	public int compare(GenericEntity e1, GenericEntity e2) {
		...
		/* if (internal) { EnhancedEntity eh1 = (EnhancedEntity)e1; EnhancedEntity eh2 = (EnhancedEntity)e1; Long rid1 = eh1.runtimeId(); Long rid2 =
		 * eh2.runtimeId();
		 * 
		...
		for (Property property : t1.getProperties()) {
			...
			assemblyComparison.pushElement(
					p -> new TraversingPropertyModelPathElement(p, v1, propertyType.getActualType(v1), e1, t1, property, property.isAbsent(e1)));
