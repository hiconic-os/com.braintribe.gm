...
public class VDE {
	...
	/**
	 * Evaluates an object with the respect to a Context (In theory, the context
	 * will include all the preset values in a default context, add optionally
	 * more)
	 * 
	 * @param object
	 *            The object that needs to be evaluated
	 * @param contextBuilder
	 *            A VdeContextBuilder that has been filled with valid aspects
	 *            and experts.
	...
	public static Object evaluate(Object object) throws VdeRuntimeException {
		return evaluate().withRegistry(registryBuilder().defaultSetup()).forValue(object);
	}

	/**
	 * Evaluates an object with the preset values of a VDE Context. In addition
	 * to the provided aspect and its value.
	 * 
	...
	public static <T, A extends VdeContextAspect<? super T>> Object evaluateWith(Class<A> aspect, T value, Object object) throws VdeRuntimeException {
		return evaluate().withRegistry(registryBuilder().defaultSetup()).with(aspect, value).forValue(object);
	}

	/**
	 * Evaluates an object with the preset values of a VDE Context. In addition
	 * to the provided concrete evaluator.
	 * 
	 ...
	 * @return evaluated object
	 */
	public static <D extends ValueDescriptor> Object evaluateWithConcreteExpert(Class<D> vdType, ValueDescriptorEvaluator<? super D> vdEvaluator, Object object) throws VdeRuntimeException {
		VdeRegistry registry = registryBuilder().loadDefaultSetup().withConcreteExpert(vdType, vdEvaluator).done();
		return evaluate().withRegistry(registry).forValue(object);
	}

	/**
	 * Evaluates an object with the preset values of a VDE Context. In addition
	 * to the provided abstract evaluator.
	 * 
	 ...
	 * @return evaluated object
	 */
	public static <D extends ValueDescriptor> Object evaluateWithAbstractExpert(Class<D> vdType, ValueDescriptorEvaluator<? super D> vdEvaluator, Object object) throws VdeRuntimeException {
		VdeRegistry registry = registryBuilder().loadDefaultSetup().withAbstractExpert(vdType, vdEvaluator).done();
		return evaluate().withRegistry(registry).forValue(object);
	}
	...
	/**
	 * @return A new instance of {@link VdeRegistryBuilder}
	 */
