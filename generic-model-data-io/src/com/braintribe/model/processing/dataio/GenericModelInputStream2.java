...
public class GenericModelInputStream2 extends DataInputStream implements ObjectInput, SerializationCodes {
	...
	private LinearCollectionExpert<Object, List<Object>> defaultListExpert;
	private MapExpert<Object, Object> defaultMapExpert;
	private LinearCollectionExpert<Object, Set<Object>> defaultSetExpert;
	...
	public GenericModelInputStream2(InputStream in, GmDeserializationOptions options) {
		this(in);
	...
	public GenericModelInputStream2(InputStream in, Consumer<Set<String>> requiredTypesReceiver, boolean absentifyMissingProperties) {
		this(in, requiredTypesReceiver);
		this.absentifyMissingProperties = absentifyMissingProperties;
	}

	public GenericModelInputStream2(InputStream in, Consumer<Set<String>> requiredTypesReceiver) {
		this(in);
		this.requiredTypesReceiver = requiredTypesReceiver;
	}

	public GenericModelInputStream2(InputStream in) {
		super(in);

		defaultListExpert = new LinearCollectionExpert<Object, List<Object>>(TYPE_LIST, CODE_LIST, objectExpert);
		defaultMapExpert = new MapExpert<Object, Object>(TYPE_MAP, objectExpert, objectExpert);
		defaultSetExpert = new LinearCollectionExpert<Object, Set<Object>>(TYPE_SET, CODE_SET, objectExpert);
	}
	...
	protected InputExpert<?> getInputExpert(GenericModelType type) throws IOException {
		...
		switch (typeCode) {
			...
			case listType:
				return getLinearCollectionExpert(CODE_LIST, (LinearCollectionType) type);
			case setType:
				return getLinearCollectionExpert(CODE_SET, (LinearCollectionType) type);
	...
	private InputExpert<?> getLinearCollectionExpert(byte code, LinearCollectionType collectionType) throws IOException {
		InputExpert<?> result = collectionExperts.get(collectionType);
		if (result == null) {
			InputExpert<Object> elementExpert = getInputExpert(collectionType.getCollectionElementType()).cast();
			result = new LinearCollectionExpert<Object, Collection<Object>>(collectionType, code, elementExpert);
			collectionExperts.put(collectionType, result);
		}
	...
	protected class LinearCollectionExpert<E, C extends Collection<E>> extends ComplexValueExpert<C> {
		private InputExpert<E> elementExpert;
		private LinearCollectionType collectionType;

		public LinearCollectionExpert(LinearCollectionType collectionType, byte code, InputExpert<E> elementExpert) {
			super(code);
		...
		protected C readPayloadComplex() throws IOException {
			int size = readInt();
			@SuppressWarnings("unchecked")
			C collection = (C) collectionType.createPlain();
	...
	protected class MapExpert<K, V> extends ComplexValueExpert<Map<K, V>> {
		...
		public MapExpert(MapType mapType, InputExpert<K> keyExpert, InputExpert<V> valueExpert) {
			super(CODE_MAP);
		...
		protected Map<K, V> readPayloadComplex() throws IOException {
			@SuppressWarnings("unchecked")
			Map<K, V> map = (Map<K, V>) mapType.createPlain();
