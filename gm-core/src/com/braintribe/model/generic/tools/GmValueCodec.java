...
public class GmValueCodec {
	...
	public static String[] parseEnumConstantIdentifier(String enumConstantIdentifier) {
		String[] result = enumConstantIdentifier.substring(5 /* "enum(" */, enumConstantIdentifier.length() - 1).split(",");
		if (result.length != 2)
			throw new IllegalArgumentException("Invalid enum constant format. enum(type,constant) expected, but was: " + enumConstantIdentifier);
		return result;
	}
