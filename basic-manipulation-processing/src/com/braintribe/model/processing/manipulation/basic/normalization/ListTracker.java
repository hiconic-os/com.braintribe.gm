...
/**
 * Tracks manipulations for property of type {@link List} and merges all the manipulations into either one {@link ChangeValueManipulation} or up to
 * two manipulations {@link RemoveManipulation}, {@link AddManipulation} (in this order).
 */
class ListTracker extends CollectionTracker {

	private final List<Entry> removes = newList();
	private final List<Entry> adds = newList();
	...
	public void onClearCollection() {
		setValueDirectly(newList());
	}

	@Override
	public void onChangeValue(ChangeValueManipulation m) {
		setValueDirectly(getTypeSafeCvmValue(List.class, m));
	}

	private void setValueDirectly(List<?> newValue) {
		list = newValue != null ? newList(newValue) : newList();
	}
	...
	public void onBulkInsert(AddManipulation m) {
		...
		List<Integer> keys = newList((Set<Integer>) (Set<?>) itemsToAdd.keySet());
	...
	public void onBulkRemove(RemoveManipulation m) {
		...
		NavigableMap<Integer, Object> itemsToRemove = newTreeMap(items);
	 ...
	 * <code>removes</code> is <tt>k</tt>. We then return the value <tt>i+k</tt>. (So the index in the <code>removes</code> array ( <tt>k</tt>) may be
	 * counted as the result of this call <tt>(i+k)</tt> minus the argument of this call <tt>i</tt>). For the definition of index, see
	 * {@link #countIndexInArray(List, int)}.
	 * <p>
	 * How does this make sense? For example imagine we have a remove with index <tt>1</tt>, but our accumulated <code>removes</code> already contains
	 * values <tt>0, 1, 3, 6</tt>. This means, if we want execute this remove as part of the bulk, we actually want to remove an element on the
	 * position <tt>4</tt>. How do we compute this? Well, since we have at least two elements we want to remove on positions smaller than our current
	 * position (those 0 and 1 must definitely have been removed before this one), we increase our index by <tt>2</tt> to <tt>3</tt>. But for this new
	 * value we now see that there is another position that must be deleted before, namely <tt>3</tt>, so we increase the value once more. Now we get
	 * a value <tt>4</tt>, and we see, that this is the value we are looking for.
	 * 
	 * In other words, we find index of our position in the <code>removes</code> array, and we increase our initial value by that. We iterate this
	 * process until we find such number <tt>k</tt>, that the index of <tt>i+k</tt> is <tt>k</tt>, in which case we may stop.
	 * 
	 * NOTE that this implementation is a little simpler (not using index but simply checking values one by one), which may cause it to be a bit
	 * slower in case lot's of removes are performed. This may be an opportunity for some fine-tuning if necessary.
	 */
	...
	/**
	 * Assumes the given array is sorted in non-descending order and returns the smallest index on which the new element could be inserted, so that
	 * this property is preserved. E.g. for array <tt>1, 4, 6, 7</tt>, the index of <tt>5</tt> is <tt>2</tt>, the index of <tt>6</tt> is also
	 * <tt>2</tt>.
	 * 
