...
/**
 * This class is responsible for providing a normal/canonical/compact form for manipulations. What it means is, that it takes a list of
 * {@link AtomicManipulation}s and returns a modified list which has an equivalent effect when applied (via the right "applyManipulation" invocation),
 * but is normalized in a certain sense.
 * <p>
 * Note that all "normalize" methods check whether a given manipulation is already a normalized one (by checking whether the type is
 * {@link NormalizedCompoundManipulation}) and if so, this original manipulation is returned without any normalization process being executed.
 * <p>
 * This process consists of two parts.
 * 
 * <h3>Removing redundant manipulations</h3>
 * 
 * This part takes a list of manipulations and returns a minimal equivalent sub-list of the original list. For example if a list contains an
 * Instantiation and Deletion of the same entity, these (along with any changes made to that entity) may be removed from the list without having any
 * effect on the resulting compound manipulation. This is implemented in {@link SimpleManipulationNormalizer}.
 * 
 * <h3>Summarizing collection manipulations</h3>
 * 
 * Manipulation applied on same collection/map property may be grouped together. Multiple inserts may be merged into one bulk-insert, insert and
 * remove may be both left out, <code>insert(a)</code> and <code>replace(a, b)</code> is equivalent to <code>insert(b)</code>.
 * 
...
public class Normalizer {
	...
	public static NormalizedCompoundManipulation normalize(Manipulation manipulation) {
		if (isNormal(manipulation))
			...
			return normalize(manipulation, new NormalizationContext(false));
	}

	public static NormalizedCompoundManipulation normalizeGlobal(Manipulation manipulation) {
		return normalize(manipulation, new NormalizationContext(true));
	}
